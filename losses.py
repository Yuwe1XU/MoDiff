...
def get_sde_loss_fn3(sde_x, sde_adj, train=True, reduce_mean=False, continuous=True,
                    likelihood_weighting=False, eps=1e-5):
  reduce_op = torch.mean if reduce_mean else lambda *args, **kwargs: 0.5 * torch.sum(*args, **kwargs)
  ...
  def loss_fn(model_x, model_adj, x, adj, u, la, u_target):
    ...
    # print("score_x:", score_x.shape)
    # score_adj = score_fn_adj(perturbed_x, perturbed_adj, flags, t, u, la)
    score_adj = score_fn_adj(perturbed_x, perturbed_adj, flags, t, u_target, perturbed_eigen)
    score_adj_mean =  torch.mean(score_adj)
    # print("score_adj_mean:", score_adj_mean)
    # print("score_adj:", score_adj.shape)
...
def get_sde_loss_fn4_D(sde_x, sde_adj, train=True, reduce_mean=False, continuous=True,
                    likelihood_weighting=False, eps=1e-5):
  reduce_op = torch.mean if reduce_mean else lambda *args, **kwargs: 0.5 * torch.sum(*args, **kwargs)
...
def get_sde_loss_fn4_DT_shift(sde_x, sde_adj, train=True, reduce_mean=False, continuous=True, likelihood_weighting=False, eps=1e-5):
  reduce_op = torch.mean if reduce_mean else lambda *args, **kwargs: 0.5 * torch.sum(*args, **kwargs)

  def loss_fn(model_x, model_adj, x, adj, u, la, la2):
    score_fn_x = get_score_fn(sde_x, model_x, train=train, continuous=continuous)
    score_fn_adj = get_score_fn_adj(sde_adj, model_adj, train=train, continuous=continuous)
    ...
    mean_eigen2,      _   = sde_adj.marginal_prob_adj(adj, t, u, la2)
    mean_eigen2 = mean_eigen2*eigen_mask

    mean_eigen = mean_eigen*eigen_mask
    perturbed_eigen = mean_eigen + std_eigen[:, None] * (z_eigen) #+ (mean_eigen-mean_eigen2)
    perturbed_eigen = perturbed_eigen*eigen_mask
    ...
    if not likelihood_weighting:
      ...
      losses_adj = torch.square(score_adj * std_eigen[:, None] + z_eigen + (mean_eigen-mean_eigen2))
      losses_adj = reduce_op(losses_adj.reshape(losses_adj.shape[0], -1), dim=-1)
